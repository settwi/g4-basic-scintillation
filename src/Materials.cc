...
namespace Materials
{
    ...
    void makeSilicon()
    {
        ...
        std::vector<G4double> zeros(SI_DET_EFF_ENERGIES.size(), 0.);

        simpt->AddProperty(kREFLECTIVITY, SI_DET_EFF_ENERGIES, zeros, useSpline);
        simpt->AddProperty(kTRANSMITTANCE, SI_DET_EFF_ENERGIES, zeros, useSpline);
